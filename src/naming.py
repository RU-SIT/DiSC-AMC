...
def train_pkl_name(
    ...
    'train_noisySignal_5_5_emb10_data.pkl'
    """
    tag = get_source(source).pkl_tag
    suffix = f"_{feature_tag}" if feature_tag else ""
    if tag:
        return f"train_{tag}_{noise_mode}_{n_bins}_{top_k}{suffix}_data.pkl"
    return f"train_{noise_mode}_{n_bins}_{top_k}{suffix}_data.pkl"
...
def test_pkl_name(
    ...
    'test_noisySignal_5_5_emb10_data.pkl'
    """
    tag = get_source(source).pkl_tag
    suffix = f"_{feature_tag}" if feature_tag else ""
    if tag:
        return f"test_{tag}_{noise_mode}_{n_bins}_{top_k}{suffix}_data.pkl"
    return f"test_{noise_mode}_{n_bins}_{top_k}{suffix}_data.pkl"
